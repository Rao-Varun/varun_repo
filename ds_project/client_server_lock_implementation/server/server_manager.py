...
class ServerManager(object):
    ...
    def _wait_until_clients_turn(self, client_name=None):
        """
        Function to maintain order in serving client.

        Function performs the following steps:
        1)Provide a unique token to each client.
        2)Add the client(token) in a queue
        3)Perform while loop until the client's token gets first in the queue
        4)Allow server to proceed forward to serve client
