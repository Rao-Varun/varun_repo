...
class PetsController extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($name != null) {
            ...
            $this->load->view("pets_login_basic_body", $name);
        } else {
            if (!(isset($_POST["form_name"]) and ($_POST["form_name"] == "login_client_insert" or $_POST["form_name"] == "login_service_insert")))
                $this->display_basic_template();
        }
    ...
    private function login()
    {
        ...
        if ($name) {
            $roleid = $name["roleid"];
            $this->display_required_login_page($roleid, $name);
        } else {
    ...
    private function perform_login_operation()
    {
        ...
        if ($roleid != 999) {
            $this->load->library('session');
            $fname = $this->PetsLoginModel->get_user_name();
            $username_data = array("username" => $fname, "roleid" => $roleid);
            $this->session->set_userdata($username_data);
        }

        $this->display_required_login_page($roleid, $username_data);


    }

    private function display_required_login_page($roleid, $username_data)
    {
        switch ($roleid) {
            case 1:
                $this->load->view("pets_login_basic_body", $username_data);
